...
export default function Contact() {
  return (
    <Layout id="contact-us" as="section" className={"space-y-2 font-semibold"}>
      <div className="text-Lime text-center bg-center bg-no-repeat bg-cover bg-[url('/src/assets/home/contact-bg.webp')] rounded-3xl space-y-10 py-10">
        <ShowInView>
          <h2 className="md:text-3xl text-xl lg:w-1/2 mx-auto">
        ...
        <ShowInView>
          {contact_links.map(({ title, par, icon: Icon }) => (
            <Link
              to="/contact"
              key={title}
              className="flex h-full items-center hover:[&_#arrow]:translate-x-2 [&_svg]:text-Lime justify-between rounded-3xl bg-Dark-green py-10 px-8"
            >
              <div className="flex items-center gap-4">
                <Icon className="text-4xl!" />
                <div className="space-y-2">
                  <h2 className="sm:text-2xl text-Lime">{title}</h2>
